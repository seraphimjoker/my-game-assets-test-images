...
const renderDynamicDesc = (descString, stats) => {
    ...
    // 將公式運算邏輯獨立抽出，以便支援 (tal) 解析與常規的標籤替換
    const _evaluateDynamicMatch = (match, stats) => {
        ...
        return { calculatedVal, statKeyRaw, labelStr };
    };

    // 智能處理 (tal) 總和計算替換
    const talRegex = /((?:(\d+)\s*hits?\s*)?(\d+(?:\.\d+)?)?\s*\+?\s*((?:(?:\[\[[^\]]+\]\]|\[[a-zA-Z_]+:[^\]]+\])\s*\+?\s*)*(?:\[\[[^\]]+\]\]|\[[a-zA-Z_]+:[^\]]+\])))\s*\(tal\)/gi;
    
    let processedDesc = descString.replace(talRegex, (match, fullExpr, hitsStr, baseStr, dynamicStr) => {
        let hits = hitsStr ? parseInt(hitsStr, 10) : 1;
        let base = baseStr ? parseFloat(baseStr) : 0;
        
        let dynamicTotal = 0;
        const dynRegex = /\[\[([^\]:]+):([^\]]+)\]\]|\[([a-zA-Z_]+):([^\]]+)\]/g;
        let dynMatch;
        while ((dynMatch = dynRegex.exec(dynamicStr)) !== null) {
            dynamicTotal += _evaluateDynamicMatch(dynMatch, stats).calculatedVal;
        }
        
        let grandTotal = (base + dynamicTotal) * hits;
        return `${fullExpr} (總: ${Math.floor(grandTotal)})`;
    });
    
    descString = processedDesc;
    ...
    while ((match = regex.exec(descString)) !== null) {
        ...
        const { calculatedVal, statKeyRaw, labelStr } = _evaluateDynamicMatch(match, stats);
...
const CASTER_SCALING_BUFFS = ['regen', 'queenSpikes', 'spellblight', 'frostburn']; // 定義需要鎖定施法者體質的動態狀態
    ...
    if (ex) {
        ...
        if (b.duration !== undefined) {
            if (ex.duration !== 99 && ex.duration !== 999) {
                ...
                ex.isNew = true; // 刷新狀態的標記，使其在當回合的結算中豁免扣減
            }
        }
...
// 萬用動態公式運算器
const evaluateDynamicFormula = (val, stats) => {
    if (!val) return 0;
    if (typeof val === 'number') return val;
    if (typeof val === 'object') {
        if (val.formula) return evaluateDynamicFormula(val.formula, stats);
...
const executeDynamicSkill = (state, cIdx, tIdx, tiers, eqSt, runState, applyDmgFn, skillDef) => {
  ...
  if (healFormula) {
      ...
      targets.forEach(t => {
          if (t.ref && t.ref.baseStats.hp > 0) {
              let healAmt = evaluateDynamicFormula(healFormula, cStats);
  ...
  if (shieldFormula) {
      ...
      targets.forEach(t => {
          if (t.ref && t.ref.baseStats.hp > 0) {
              let sAmt = evaluateDynamicFormula(shieldFormula, cStats);
              if (duration !== null) {
  ...
  if (damageFormula) {
      ...
      targets.forEach(t => {
          if (t.ref && t.ref.baseStats.hp > 0) {
              let dmgAmt = evaluateDynamicFormula(damageFormula, cStats);
  ...
  if (energyFormula) {
      ...
      targets.forEach(t => {
          if (t.ref && t.ref.baseStats.hp > 0) {
              let epAmt = evaluateDynamicFormula(energyFormula, cStats);
...
let ULT_DB = {};
const ULT_LOGIC = {
  'u_C001': { calcBaseDmg: (stats) => 150 + (1.2 * stats.atk), debuff: [{type: 'pDefDown', val: 0.25, duration: 2}], descFn: (stats) => { const b = Math.round(1.2*(stats?.atk||0)); const d = 150+b; return <>{FMT('對敵單體造成 3 ')}<span className="text-red-500 font-bold">hits</span>{FMT(' 150+')}{renderBonusWithTooltip(b, "text-orange-400", "1.2 物攻")}{FMT(` 傷害(總: ${d*3})，減物防25%(2T)`)}</>; } },
  'u_C002': { calcBaseDmg: (stats) => 300 + (1.0 * stats.matk), partyBuff: [{type: 'queenSpikes', stacks: 1, duration: 3}], descFn: (stats) => { const b = Math.round(1.0*(stats?.matk||0)); const d = 300+b; return <>{FMT('對敵全體造成 1 ')}<span className="text-red-500 font-bold">hit</span>{FMT(' 300+')}{renderBonusWithTooltip(b, "text-purple-400", "1.0 魔攻")}{FMT(` 傷害(總: ${d})，全體【魔后】1層(3T)`)}</>; } },
  'u_C003': { calcBaseDmg: (stats) => 100 + (0.8 * stats.atk), debuff: [{type: 'pDefDown', val: 0.50, duration: 1}, {type: 'mDefDown', val: 0.50, duration: 1}], descFn: (stats) => { const b = Math.round(0.8*(stats?.atk||0)); const d = 100+b; return <>{FMT('對敵單體造成 6 ')}<span className="text-red-500 font-bold">hits</span>{FMT(' 100+')}{renderBonusWithTooltip(b, "text-orange-400", "0.8 物攻")}{FMT(` 傷害(總: ${d*6})，減雙防50%(1T)`)}</>; } },
  'u_C004': { calcHeal: (stats) => 150 + (1.2 * stats.matk), partyBuff: [{type: 'holyBless', stacks: 2, duration: 3}], descFn: (stats) => { const b = Math.round(1.2*(stats?.matk||0)); const heal = 150+b; return <>{FMT('全體回復 150+')}{renderBonusWithTooltip(b, "text-purple-400", "1.2 魔攻")}{FMT(` 生命(總: ${heal})，全體【聖祝】2層(3T)`)}</>; } },
  ...
  'u_C006': { calcBaseDmg: (stats) => 150 + (0.6 * stats.atk), partyBuff: [{type: 'monarch', duration: 2}], postEffect: (char, draft, addPopup) => { draft.enemies.forEach((e, idx) => { if (e && e.baseStats.hp > 0) { let existing = e.buffs.find(b => b.type === 'burn'); if (existing) { existing.stacks = Math.min(99, existing.stacks + 3); existing.duration = 3; existing.isNew = true; } else { e.buffs.push({type: 'burn', stacks: 3, duration: 3, isNew: true, casterId: char.id, casterSide: 'party'}); } addPopup('enemy', idx, 'burn', '', {isBuff: true, isDebuff: true}); } }); }, descFn: (stats) => { const b = Math.round(0.6*(stats?.atk||0)); const d = 150+b; return <>{FMT('對敵單體造成 5 ')}<span className="text-red-500 font-bold">hits</span>{FMT(' 150+')}{renderBonusWithTooltip(b, "text-orange-400", "0.6 物攻")}{FMT(` 傷害(總: ${d*5})，敵全體3層【灼傷】(3T)，我方全體【烽火】(2T)`)}</>; } },
  'u_C007': { calcBaseDmg: (stats, attackerRef, draft) => { let totalSt=0; draft?.party?.forEach(p => { if (p?.baseStats?.hp>0) { let t = p.buffs?.find(b=>b.type==='tide'); if(t) totalSt+=t.stacks; }}); return ((200+1.0*stats.atk+2.5*stats.matk)*(1+totalSt*0.1))/4; }, postEffect: (attackerRef, draft) => { draft?.party?.forEach(p => { if (p) p.buffs = p.buffs?.filter(b=>b.type!=='tide'); }); }, descFn: (stats, attackerRef, state) => { let totalSt=0; state?.party?.forEach(p => { if (p?.baseStats?.hp>0) { let t = p.buffs?.find(b=>b.type==='tide'); if(t) totalSt+=t.stacks; }}); const b1=Math.floor(1.0*(stats?.atk||0)); const b2=Math.floor(2.5*(stats?.matk||0)); const multi=1+totalSt*0.1; const totalDmg=Math.floor((200+b1+b2)*multi); return <>{FMT('對敵單體造成 4 ')}<span className="text-red-500 font-bold">hits</span>{FMT(` (200+`)}{renderBonusWithTooltip(b1, "text-orange-400", "1.0 物攻")}{FMT('+')}{renderBonusWithTooltip(b2, "text-purple-400", "2.5 魔攻")}{FMT(`) x (每層潮汐提升10%，當前: x${multi.toFixed(1)}) 的總傷害(總計: ${totalDmg})，移除全體【潮汐】`)}</>; } },
  'u_C008': { calcDebuffAll: (stats) => [{type: 'pAtkDown', val: 0.2, duration: 2}, {type: 'mAtkDown', val: 0.2, duration: 2}, {type: 'pDefDown', val: 0.4, duration: 2}, {type: 'mDefDown', val: 0.4, duration: 2}], postEffect: (attackerRef, draft, addPopupFn) => { let wg = attackerRef?.buffs?.find(b=>b.type==='windGuard'); let st = wg?wg.stacks:0; let cStats = getStats(attackerRef, true, {}, {}, {}); let sAmt = Math.floor(200+0.4*cStats.pdef+0.4*cStats.mdef+st*50); draft.party.forEach((p, idx) => { if (p.baseStats?.hp>0) { p.baseStats.permShield = (p.baseStats.permShield || 0) + sAmt; p.baseStats.shield = (p.baseStats.permShield || 0) + (p.baseStats.tempShields || []).reduce((sum, ts) => sum + ts.amt, 0); addPopupFn('party', idx, `護盾 +${sAmt}`, 'text-white font-bold scale-110 drop-shadow-md'); }}); }, descFn: (stats, attackerRef) => { let wg = attackerRef?.buffs?.find(b=>b.type==='windGuard'); let st = wg?wg.stacks:0; const b1=Math.floor(0.4*(stats?.pdef||0)); const b2=Math.floor(0.4*(stats?.mdef||0)); return <>{FMT('敵全體雙攻降20%、雙防降40%(2T)，全體 200+')}{renderBonusWithTooltip(b1, "text-yellow-400", "0.4 物防")}{FMT('+')}{renderBonusWithTooltip(b2, "text-cyan-400", "0.4 魔防")}{FMT('+')}{renderBonusWithTooltip(st*50, "text-green-300", "每層風護 50 點")}{FMT(` (總計: ${200+b1+b2+st*50}) 護盾`)}</>; } }
};
...
function getStats(char, isPlayer = false, tiers = {}, charEquips = {}, runState = {}) {
  ...
  calculatedCrit = Math.max(0, Math.min(calculatedCrit, 1.0));
  ...
  return {
    ...
    mdef: mDefFinal, 
    crit: calculatedCrit, 
    da: Math.max(0, ((bStats.da || 0.1) + mods.da + flat.da) * (isPlayer ? campMult : 1.0)), 
    ta: Math.max(0, ((bStats.ta || 0.02) + mods.ta + flat.ta) * (isPlayer ? campMult : 1.0)),
      ...
      try {
        ...
        if (ultdbRes && typeof ultdbRes === 'object') {
            Object.keys(ultdbRes).forEach(key => {
                let logic = ULT_LOGIC[key] || {};
                let jsonDef = ultdbRes[key];
                
                // 以 JSON 為主體覆蓋 logic
                let merged = { ...logic, ...jsonDef };
                
                // 如果 JSON 裡面自帶了描述，就強制關閉硬編碼的 descFn 函式，由動態渲染器完全接手
                if (jsonDef.desc) {
                    delete merged.descFn;
                }
                // 如果 JSON 定義了傷害公式或數值，拔除硬編碼的舊傷害計算
                if (jsonDef.damageFormula || jsonDef.damageScaling || jsonDef.damage) {
                    delete merged.calcBaseDmg;
                }
                // 如果 JSON 定義了治療公式，拔除硬編碼的舊治療計算
                if (jsonDef.healFormula || jsonDef.healScaling || jsonDef.heal) {
                    delete merged.calcHeal;
                }
                // 如果 JSON 中明確定義了給予狀態，拔除舊的 buff 寫法
                if (jsonDef.applyBuffs || jsonDef.applyDebuffs) {
                    if (jsonDef.applyBuffs) { delete merged.partyBuff; delete merged.calcPartyBuff; }
                    if (jsonDef.applyDebuffs) { delete merged.debuff; delete merged.debuffAll; delete merged.calcDebuffAll; }
                }
...
const executeAttackPhase = async () => {
    ...
    for (let i = 0; i < draft.party.length; i++) {
      ...
      if (useUlt) {
        ...
        let pBuffs = ultDef.applyBuffs || ultDef.partyBuff || (ultDef.calcPartyBuff ? ultDef.calcPartyBuff(cStats) : null);
        let eDebuffs = ultDef.applyDebuffs || ultDef.debuffAll || ultDef.debuff || (ultDef.calcDebuffAll ? ultDef.calcDebuffAll(cStats) : null);
        
        let dmgFormula = ultDef.damageFormula || ultDef.damageScaling || ultDef.damage;
        let healFormula = ultDef.healFormula || ultDef.healScaling || ultDef.heal;
        
        let ultType = ultDef.type;
        if (!ultType) {
             if (dmgFormula) ultType = 'damage';
             else if (healFormula) ultType = 'heal';
             else ultType = 'support';
        }
        ...
        if (ultType === 'support') {
          ...
          let healAmt = healFormula ? evaluateDynamicFormula(healFormula, cStats) : (ultDef.calcHeal ? ultDef.calcHeal(cStats) : Math.floor(cStats.matk * (ultDef.multiplier || 1.0)));
          draft.party.forEach((p, pIdx) => {
             if(p.baseStats.hp > 0) {
                let res = applyHeal(p, healAmt, true, globalStorage.charTiers, globalStorage.charEquips, runState, cStats);
                if(pBuffs) pBuffs.forEach((db, dbIdx) => { applyUltBuff(p, db, char.id, 'party'); addPopup('party', pIdx, db.type, '', {isBuff:true, isDebuff: false, delay: dbIdx * 300}); });
                let color = res.isCrit ? 'text-green-300 font-black scale-125 drop-shadow-[0_0_8px_rgba(74,222,128,0.8)]' : 'text-green-400 font-bold scale-110';
          ...
          if (ultDef.postEffect) ultDef.postEffect(char, draft, addPopup);
           ...
           for (let target of targets) {
             ...
             for(let h=0; h < (ultDef.hits || 1); h++) {
               let baseDmgOverride = dmgFormula ? evaluateDynamicFormula(dmgFormula, cStats) : (ultDef.calcBaseDmg ? ultDef.calcBaseDmg(cStats, char, draft) : null);
             ...
             if (eDebuffs) eDebuffs.forEach((db, dbIdx) => { applyUltBuff(realTargetRef, db, char.id, 'party'); addPopup('enemy', target.originalIdx, db.type, '', {isBuff:true, isDebuff: true, delay: dbIdx * 300}); });
    ...
    for (let i = 0; i < draft.enemies.length; i++) {
      ...
      for (let a = 0; a < actionCount; a++) {
          ...
          if (triggeredSkillId) {
              ...
              const shieldDef = sDef.shieldScaling || sDef.shield || sDef.shieldFormula || sDef.shieldVal;
              if (shieldDef) {
                  let shieldTargets = shieldDef.target ? getTargets(shieldDef.target, defaultTargetObj) : (sDef.target ? getTargets(sDef.target, defaultTargetObj) : baseTargets);
                  let shieldBase = 0;
                  let duration = null;

                  if (typeof shieldDef === 'object' && shieldDef !== null) {
                      if (shieldDef.duration !== undefined) duration = shieldDef.duration;
                      shieldBase = evaluateDynamicFormula(shieldDef, eStats);
                      
                      // 容錯支援：如果動態解析失敗，回退使用舊式 factors 相加
                      if (shieldBase <= 0 && shieldDef.factors && Array.isArray(shieldDef.factors)) {
                          shieldBase += shieldDef.factors.reduce((sum, f) => sum + (eStats[f.stat] || 0) * (f.value || 0), 0);
                      }
                  } else {
                      shieldBase = evaluateDynamicFormula(shieldDef, eStats);
                  }

                  if (shieldBase <= 0 && factorVal > 0 && !sDef.damage && !sDef.healing) {
                  ...
                  if (shieldBase > 0) {
                      for (let t of shieldTargets) {
                          ...
                          if (duration !== null) {
                              t.ref.baseStats.tempShields = t.ref.baseStats.tempShields || [];
                              t.ref.baseStats.tempShields.push({ amt: Math.floor(shieldBase), duration: duration });
                          } else {
    ...
    const processBuffs = (entity, side, idx, isPlayer, draftState) => {
      ...
      entity.buffs = entity.buffs.map(b => {
         ...
         if ((bDef.mech === 'duration' || bDef.mech === 'stack_duration') && b.duration !== undefined && b.duration !== 99 && b.duration !== 999) {
             let shouldReduce = true;
             let nextIsNew = b.isNew;
             
             if (b.isNew) {
                 // 敵方施加給玩家的回合制狀態，當回合豁免扣減，確保它能存活到下回合玩家行動
                 if (b.casterSide === 'enemy' && side === 'party') {
                     shouldReduce = false;
                 }
                 nextIsNew = false;
             }
             ...
             if (shouldReduce) {
                 return { ...b, duration: b.duration - 1, isNew: nextIsNew };
             } else {
                 return { ...b, isNew: nextIsNew };
             }
         }
         return { ...b, isNew: false };
      }).filter(b => {
                             ...
                             <div className="bg-gray-800/50 p-5 rounded-xl border border-red-900/30 shadow-inner">
                                 ...
                                 <div className="flex gap-4">
                                     ...
                                     <div className="flex-1 min-w-0">
                                         ...
                                         {renderRelatedBuffs([ ...(ultDef.selfBuff || []).map(b=>b.type), ...(ultDef.partyBuff || []).map(b=>b.type), ...(ultDef.debuffAll || []).map(b=>b.type), ...(ultDef.debuff || []).map(b=>b.type), ...(ultDef.applyBuffs || []).map(b=>b.type), ...(ultDef.applyDebuffs || []).map(b=>b.type) ], cStats)}
                                     </div>
                                 </div>
                             </div>
                         )}
             ...
             <div className="mb-6 bg-gray-900 p-4 rounded-lg border border-gray-700">
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-6 text-sm text-gray-300 font-mono">
                   {[ {label: '❤️ 最大生命', key: 'maxHp', isPct: false, color: 'text-white'}, 
                      ...
                      if (st.key === 'da' || st.key === 'ta' || st.key === 'crit') {
                          eqV = Math.max(0, Math.min(1.0, eqV));
                          basV = Math.max(0, Math.min(1.0, basV));
                      }
                     ...
                     <div className="bg-gray-900 p-3 rounded-lg border border-gray-700 mb-4 shrink-0">
                         ...
                         <div className="flex items-start gap-3">
                             ...
                             <div className="flex-1 min-w-0">
                                 ...
                                 <div className="text-[10px] text-gray-300 leading-snug">{ULT_DB[battleUnitDetail.ult].descFn ? ULT_DB[battleUnitDetail.ult].descFn(getStats(battleUnitDetail, !!battleUnitDetail.role, globalStorage.charTiers, globalStorage.charEquips, runState), battleUnitDetail, battleState) : renderDynamicDesc(ULT_DB[battleUnitDetail.ult].desc, getStats(battleUnitDetail, !!battleUnitDetail.role, globalStorage.charTiers, globalStorage.charEquips, runState))}</div>
                             </div>
                         </div>
                     </div>
                 )}
                         ...
                         {(() => {
                             ...
                             const renderStat = (label, key, color, isPct = false) => {
                                 ...
                                 if (isPct && (key === 'da' || key === 'ta' || key === 'crit')) {
                                     ...
                                     baseVal = Math.max(0, Math.min(1.0, baseVal));
                                 }
                     ...
                     <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-hide">
                         {battleUnitDetail.buffs && battleUnitDetail.buffs.length > 0 ? (
                             battleUnitDetail.buffs.map((b, i) => {
                                 ...
                                 const isDebuff = bDef.category === 'debuff' || (bDef.category !== 'buff' && (bDef.effectType === 'silence' || bDef.effectType === 'control' || b.type.toLowerCase().includes('down') || bDef.dot || b.isDebuff === true));
